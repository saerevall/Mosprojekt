...
public class WorldUpdate {
    ...
    public WorldUpdate(){
        ...
        this.wall = new Floor(20f, Gdx.graphics.getHeight() ,new Vector2(Gdx.graphics.getWidth() - 10, 0));

        ballBody = world.createBody(ball.circleBody);
        floorBody = world.createBody(floor.floorBody);
        roofBody = world.createBody(roof.floorBody);
        wallBody = world.createBody(wall.floorBody);

        ballBody.createFixture(ball.circleDef);
        floorBody.createFixture(floor.floorShape, 0.0f);
        roofBody.createFixture(roof.floorShape, 0.0f);
        wallBody.createFixture(wall.floorShape, 0.0f);
    }
